...
function Article({ newArticle, fullArticle }) {
  ...
  const showBody = fullArticle ? (
    <div className={`${styles['post-body']}`}>
      <ReactMarkdown children={bodyMarkdown} remarkPlugins={[commentGfm]} />
    </div>
  ) : null
  const buttonDelete =
    fullArticle && author.username === username ? (
      <Popconfirm
        ...
        placement='rightTop'
      >
        <button className={`${styles.delete} ${styles['article-btn']}`}>Delete</button>
      </Popconfirm>
    ) : null
  const buttonEdit =
    fullArticle && author.username === username ? (
  ...
  return (
    <>
      <div className={`${styles['article-title']}`}>
        ...
        <p className={`${styles['article-likeNum']}`}>{numberLikes}</p>
      ...
      <div className={`${styles['article-tags']}`}>{tags}</div>
