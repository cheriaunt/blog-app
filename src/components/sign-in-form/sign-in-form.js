...
function SignIn() {
  ...
  return (
    <form onSubmit={handleSubmit((data) => onSubmit(data))}>
      <h2 className={styles['form-header']}>Sign In</h2>
      <ul className={styles.inputs}>
        <li className={styles['inputs-item']}>
          ...
          {serverErrors['email or password'] ? (
            <p className={styles.errorInfo}>email or password {serverErrors['email or password']}</p>
          ) : null}
        </li>
        <li className={styles['inputs-item']}>
      ...
      <div className={styles.info}>
        <span className={styles['info-grey']}>`Don't have an account?</span>
        <Link to='/sign-up' className={styles['info-blue']}>
          {' '}
          Sign Up.
        </Link>
      </div>
